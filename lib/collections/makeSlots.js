...
items.before.update(function (userId, doc, fieldNames, modifier, options) {
    ...
    for (var f = 0; f < subtarefas.length; f++) {
        ...
        if ((curSlot != undefined) && (curSlot.length > 0)) {
            //console.log("Already has slots", curSlot);
            console.log("Update Already has slots");
...
Meteor.users.before.update(function (userId, doc, fieldNames, modifier, options) {
    ...
    if ((doc) && (doc.profile)) {
        if ((!doc.profile.ranking) && ((doc.profile.firstName) || (modifier.$set["profile.firstName"]))) {
            var name = doc.profile.firstName || modifier.$set["profile.firstName"];
            rank = Utils.getRanking(name);
            ...
            modifier.$set["profile.ranking"] = rank
            delete(modifier.$unset["profile.ranking"])
